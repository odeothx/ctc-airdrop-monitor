...
RPC_URLS = {
    ...
    "mainnet_remote": "https://rpc.mainnet.creditcoin.network",
...
CONTRACT_ADDRESSES = {
    # Mainnet: 첫 번째 컨트랙트를 기본으로 사용 (실제로는 모든 컨트랙트 스캔)
    "mainnet": MAINNET_CONTRACTS[0],
    "mainnet_remote": MAINNET_CONTRACTS[0],
    # Testnet: 사용자 지정 주소 (테스트용)
    "testnet": "0x824c6A8FB6311379bf8Ba10e90C1843B16E3A4cE",
}
...
# Known Token Addresses (Spacecoin)
KNOWN_TOKENS = {
...
class AirdropMonitor:
    ...
    def check_all_contracts_for_wallet(
        ...
        for i, contract in enumerate(self.contracts):
            ...
            for campaign_name in known_names:
                try:
    ...
    if discovered_campaigns:
        ...
        if campaigns_with_rewards:
            ...
            for campaign, rewards in campaigns_with_rewards:
                print(f"\n--- Campaign: {campaign['campaign_hash'][:20]}... ---")
